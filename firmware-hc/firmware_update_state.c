...
static struct hc_firmware_info s_update_firmware;

void update_firmware_cmd(u8 *data, int data_len)
{
	if (data_len < sizeof(s_update_firmware)) {
		finish_command_resp(INVALID_INPUT);
		return;
	}
	memcpy(&s_update_firmware, data, sizeof(s_update_firmware));
}
...
void firmware_update_write_block_complete(u32 error)
{
	...
	case DS_ERASING_PAGES:
		g_cmd_data.erase_flash_pages.index++;
		update_state_progress(DS_ERASING_PAGES, 0, g_cmd_data.erase_flash_pages.index - g_cmd_data.erase_flash_pages.min_page, get_state_max_progress(DS_ERASING_PAGES, 0));
		if (g_cmd_data.erase_flash_pages.index > g_cmd_data.erase_flash_pages.max_page) {
			enter_state(DS_FIRMWARE_UPDATE);
		} else {
			u8 *addr = (u8 *)flash_sector_to_addr(g_cmd_data.erase_flash_pages.index);
...
static void erase_flash_pages_cmd(u8 *data, int data_len)
{
	...
	case HC_BOOT_BOOTLOADER_MODE:
		g_cmd_data.erase_flash_pages.index = 5;
		g_cmd_data.erase_flash_pages.min_page = 5;
		g_cmd_data.erase_flash_pages.max_page = 7;
		break;
	case HC_BOOT_APPLICATION_MODE:
		g_cmd_data.erase_flash_pages.index = 2;
		g_cmd_data.erase_flash_pages.min_page = 2;
		g_cmd_data.erase_flash_pages.max_page = 4;
		break;
	default:
		finish_command_resp(INVALID_STATE);
		return;
	}
	int temp[] = {g_cmd_data.erase_flash_pages.max_page -
		g_cmd_data.erase_flash_pages.min_page};
...
void write_flash_cmd(u8 *data, int data_len)
{
	...
	g_cmd_data.write_flash.addr = (u32)dest;
	dest += write_addr_base;
	data += 4;
	data_len -= 4;
	memcpy(g_cmd_data.write_flash.block, data, data_len);
	g_cmd_data.write_flash.sz = (u16)data_len;

	if (!in_memory_range((u32)dest, write_addr_base, write_addr_len) ||
		!in_memory_range((u32)(dest + g_cmd_data.write_flash.sz - 1), write_addr_base, write_addr_len)) {
		finish_command_resp(INVALID_INPUT);
	} else {
		if (!flash_write(dest, g_cmd_data.write_flash.block, g_cmd_data.write_flash.sz)) {
			finish_command_resp(INVALID_STATE);
		}
	}
}
...
void switch_boot_mode_cmd(u8 *data, int data_len)
{
	...
	case HC_BOOT_BOOTLOADER_MODE:
		...
		if (crc != s_update_firmware.firmware_crc) {
			finish_command_resp(INVALID_STATE);
			return;
		}
